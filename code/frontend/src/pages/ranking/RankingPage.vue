        ...
        <div v-for="(player, index) in activeRanking" :key="player.id"
          ...
          <div class="flex items-center gap-6">
            ...
            <div class="w-14 h-14 rounded-2xl bg-slate-200 overflow-hidden shadow-sm border border-slate-100">
              <img :src="player.photo_avatar_filename ? `${API_DOMAIN_URL}/storage/photos_avatars/${player.photo_avatar_filename}` : '/default.jpg'" 
...
// **Usar variável para API DOMAIN**
const API_DOMAIN = import.meta.env.VITE_API_DOMAIN
const API_DOMAIN_URL = `http://${API_DOMAIN}/api`
...
const fetchGlobal = async () => {
  loading.value = true
  try {
    const url = `${API_DOMAIN_URL}/ranking/global${selectedVariant.value ? '?variant=' + selectedVariant.value : ''}`
...
const fetchPersonal = async () => {
  ...
  try {
    const res = await axios.get(`${API_DOMAIN_URL}/ranking/personal`, {
      headers: {
...
// Computed
const activeRanking = computed(() => currentTab.value === 'wins' ? rankings.value.by_wins : rankings.value.by_matches)
